    ...
    override suspend fun getCurrentLocation(): Location? {
        ...
        return suspendCancellableCoroutine { cont ->
            fusedLocationProviderClient.lastLocation.apply {
                if (isComplete) {
                    if (isSuccessful) {
                        cont.resume(result) { _, _, _ -> } // Resume coroutine with location result
                    } else {
                        cont.resume(null) { _, _, _ -> } // Resume coroutine with null location result
                    }
                    return@suspendCancellableCoroutine
                }
                addOnSuccessListener {
                    cont.resume(it) { _, _, _ -> }  // Resume coroutine with location result
                }
